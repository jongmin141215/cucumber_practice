describe User do
  ...
  describe 'password encryption' do
    it 'encrypts password' do
      ...
      expect(user.password_digest).not_to eql('test')
      expect(user.password_digest.class).to eq(BCrypt::Password)
    end

    it 'raises error when password_confirmation does not match' do
  ...
  describe 'user authentication' do
    before do
      @user = User.create(name: 'Jongmin', email: 'test@test.com', password: 'password', password_confirmation: 'password')
    end
    it 'succedes with valid credentials' do
